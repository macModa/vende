...
class FirebaseCartRepositoryAdapter implements ICartRepository {
  ...
  @override
  Stream<List<Order>>? currentUserOrdersStream() {
    final stream = _repository.currentUserOrdersStream();
    return stream?.map((orders) => orders.cast<Order>());
  }
}
...
class UnifiedCartOperationsNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  UnifiedCartOperationsNotifier(this._repository, Ref ref) : super(const AsyncValue.data(null));
